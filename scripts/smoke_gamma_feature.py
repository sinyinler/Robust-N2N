# -*- coding: utf-8 -*-
"""不依赖数据集的 raw 域局部 Gamma feature 分支 smoke test。"""
...
def main() -> None:
    ...
    raw_n1 = torch.rand(2, 1, 64, 64, device=device) * 100.0
    raw_n2 = torch.rand_like(raw_n1) * 100.0
    n1 = torch.log1p(raw_n1)
    n2 = torch.log1p(raw_n2)

    region_generator = torch.Generator(device=device).manual_seed(20_188)
    noise_generator = torch.Generator(device=device).manual_seed(40_188)
    ...
    assert bool(((cvs >= 0.025) & (cvs <= 0.075)).all())
    raw_ratio = (
        (torch.expm1(corrupted) * hidden).sum()
        / (torch.expm1(n1) * hidden).sum().clamp_min(1e-12)
    )

    # Gamma 模式保持单通道；region map 不传入 student。
    ...
    with tempfile.TemporaryDirectory() as tmp:
        checkpoint = Path(tmp) / "gamma_feature.pth"
    ...
    print(
        f"[OK] raw-domain local Gamma feature smoke device={device} "
        f"hidden={float(hidden.mean()):.3f} "
        f"cv={float(cvs.mean()):.4f} raw_ratio={float(raw_ratio):.6f} "
        f"feature={float(feature_loss.detach()):.4f} checkpoint={loaded}"
    )
